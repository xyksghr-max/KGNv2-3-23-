...
- 当前主线已进入 `T3.5b inference-side enhancement`，分支从文档化的 `T3.4 nearest_cost` 基线重新打开。
- `T3.5a nearest_conf` 的负向结果不会改变 T1 的迁移硬边界。
- 也就是说，后续若引用本文件，仍应把它当作“资产边界和迁移风险说明”，而不是“当前主线直接照搬 KGN-Pro-main 的许可”。
