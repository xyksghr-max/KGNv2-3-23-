"""
Minimal perspective camera used by the vendored EPro-PnP helper.
"""

import torch

from .common import quaternion_to_rot_mat, skew, yaw_to_rot_mat


def project_a(x3d, pose, cam_mats, z_min):
    if pose.size(-1) == 4:
...
def project_b(x3d, pose, cam_mats, z_min):
    if pose.size(-1) == 4:
    ...
    def get_quaternion_transfrom_mat(quaternions):
        ...
        transform_mat = torch.stack(
            ( i,  j,  k,
             ...
             -j,  i, -w),
            dim=-1)
        return transform_mat.reshape(quaternions.shape[:-1] + (4, 3))
